...
function ResourceEmbed({ resource }) {
  if (!resource) {
    return (
      <div className="bg-[#fbe3bb] rounded-lg border border-[#977968] p-4 text-center">
        <p className="text-[#533933]">No resource available for this step.</p>
      </div>
    );
  }
  ...
  if (resource.startsWith("http")) {
    ...
    if (resource.includes("youtube.com") || resource.includes("youtu.be")) {
      ...
      return (
        <div className="border border-[#977968] rounded-lg overflow-hidden">
          <div className="bg-[#fbe3bb] px-4 py-2 border-b border-[#977968] flex justify-between items-center">
            <span className="font-medium text-sm truncate text-[#533933]">
              {resource}
            </span>
            <a
              href={resource}
              className="text-[#533933] hover:text-[#977968] text-sm ml-2 font-medium"
            >
              Open in new tab
            </a>
          </div>
          <iframe title="Resource" src={resource} className="w-full h-96" />
        </div>
      );
    }
  } else {
    // We assume it's text
    return (
      <div className="bg-white border border-[#977968] rounded-lg p-4 prose max-w-none">
        <ReactMarkdown remarkPlugins={[remarkGfm]}>{resource}</ReactMarkdown>
      </div>
    );
  }
}
  ...
  async function handleSubmitAnswer(selectedOption) {
    ...
    if (isCorrect) {
      ...
      if (attemptNumber < 3) {
        ...
        setFeedback(
          `Incorrect. The correct answer is: ${quizData.options[correctAnswerIdx]}`
        );
        setFeedbackType("error");
      }
    }
    ...
    <div className="mt-10 ml-10 mr-10 mb-10">
      ...
      {plans.length === 0 && !loading && (
        <div className="text-center bg-gray-50 rounded-lg p-8 border-2 border-dashed border-gray-300">
          ...
          <h3 className="mt-2 text-sm font-medium text-[#533933]">
            No study plans found
          </h3>
          <p className="mt-1 text-sm text-gray-500">
            Get started by creating your first study plan.
          </p>
          <div className="mt-6 text-[#533933]">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
              <h3 className="text-lg font-medium text-[#533933]">Your Plans</h3>
            </div>
              ...
              {plans.map((p) => (
                ...
                >
                  <div className="flex-1">
                    <h4 className="font-medium text-[#533933] mb-1">
                      {p.title}
                    </h4>
                    <p className="text-xs text-gray-500">Number: {p.id}</p>
                  </div>
                  <button
            ...
            {plans.length > 0 && (
              <div className="p-4 bg-gray-50 border-t border-gray-200">
                <Link
                  to="/create-plan"
                  className="flex justify-center text-[#533933] items-center text-primary-600 hover:text-primary-800"
                >
                  <svg
                  ...
                  <span className="text-[#533933]">Create New Plan</span>
                </Link>
              </div>
            )}
          </div>
          ...
          {!loading && !selectedPlan && plans.length > 0 && (
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              ...
              <p className="mt-1 text-gray-500">
                Choose a plan from the list to view its content and start
                learning.
              </p>
            </div>
          )}
